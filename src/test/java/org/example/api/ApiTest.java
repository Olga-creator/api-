...
@CucumberOptions(glue = "org.example.steps",
...
//        pet.setName(name);
//
//        Allure.addAttachment("Отправляемое тело", ContentType.JSON.toString(), new Gson().toJson(pet));
//
...
//        logToAllure("Принятый объект", ContentType.JSON.toString(), new Gson().toJson(actual).getBytes());
//        Assert.assertEquals(actual.getName(), pet.getName());
//
//    }
//    @Attachment(value = "{name}", type = "{type}")
//    public static byte[] logToAllure(String name, String type, byte[] content) {
//        return content;
//    }
}
